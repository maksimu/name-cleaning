...
parser.add_argument('-f','--fncol', type=int, help='First name column number',required=True)
parser.add_argument('-l','--lncols', type=int, help='Last name column number', required=True)
parser.add_argument('-m','--mncol', type=int, help='Middle name column number', required=False, default=-1)
parser.add_argument('-s','--sncol', type=int, help='Suffix name column number', required=False, default=-1)
parser.add_argument('-wn','--worksheetname',help='Worksheet name that will be used to extract a string name and also used to write extracted name parts. '
...
while curr_row < num_rows:
    ...
    #print "--> " ,clear_name
    worksheet_to_write = workbook_to_write.get_sheet(4)
